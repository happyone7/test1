...
namespace Soulspire.UI
{
    ...
    public class TreasureChoiceUI : MonoBehaviour
    {
        ...
        private void BuildUI(List<TreasureChoiceData> choices)
        {
            ...
            {
                var card = CreateCard(choices[i], cardContainer.transform, cardWidth, cardHeight);
                _cardObjects.Add(card);
            }
        }

        private GameObject CreateCard(TreasureChoiceData data, Transform parent, float width, float height)
        {
        ...
        private void OnCardClicked(TreasureChoiceData choice)
        {
            if (!_isShowing) return;
