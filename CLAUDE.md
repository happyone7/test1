...
- **스프린트 종료 시 DevPD 필수 리뷰**: 해당 스프린트에서 발생한 문제(버그, QA 실패, 프로세스 장애)를 분석하고, 원인이 된 문서(skill, agent, CLAUDE.md, prefab-protocol 등)의 정책/절차 불일치·누락·모호함을 식별하여 수정 적용. 문서 간 경로·용어·역할 분담 일관성 검증 포함
