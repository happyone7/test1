...
2. **실무자 자체 QA**: 프로그래밍팀장, UI팀장 등은 작업 후 자체 QA
3. **QA 조기 착수**: 각 팀장 작업 단위 완료 시 즉시 해당 부분 QA
4. **QA 통과 후 빌더에게 빌드 명령**: 모든 작업 완료 + QA 통과 → 빌더가 빌드
5. **UI 작업은 반드시 UI팀장 경유**: UI 수정/생성은 unity-ui-developer 에이전트 사용
6. **UI팀장이 목업 + Unity 구현 모두 담당** (프로토타입 단계)
7. **UI 명세는 PPT로 작성** (기획팀장이 디자인 없는 레이아웃 명세 담당)
8. **TA↔UI 역할 분리**: TA는 UI 컨셉/이미지 리소스만, UI 시스템 구현은 UI팀장 담당
9. **SO 수치 수정은 기획팀장 담당**: 코드 변경 없이 SO 수치만 바꾸는 작업은 기획팀장이 직접 수행
