...
> **총괄PD 확정 (2026-02-17)**: 타워 추가 획득은 보물상자를 통해서만 가능. 인게임 중 랜덤 획득(주로 보스 드롭), Hub 보물상자 메뉴에서 오픈.
...
| 오픈 장소 | 인게임 (일시정지) | **Hub 보물상자 메뉴** |
| 획득 경로 | 웨이브 클리어 + Node 드롭 + 보스 드롭 | **보스 드롭(확정) + Node 드롭(낮은 확률)** |
...
| 드롭 소스 | 확률 | 비고 |
|----------|------|------|
| 보스 Node 처치 | **100% (확정)** | 모든 스테이지의 보스를 잡으면 반드시 1개 |
| 일반 Node 처치 | **1~3%** | 기본 1%, 웨이브 진행에 따라 소폭 증가 |
...
| 스테이지 클리어 (보스 처치) | 1 | 0.5~1.5 | **~1.5~2.5개** |
...
- **Gold**: 영구 업그레이드(스킬 트리)에 사용 -> 전체적인 파워 상승
...
| P0 | Hub 보물상자 오픈 UI (3지선다 타워 카드) | Sprint 4 |
