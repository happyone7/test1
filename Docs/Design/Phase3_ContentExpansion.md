...
| **GameManager** | stages[] 배열로 멀티 Floor 지원 가능 | StartRun(stageIndex), OnRunEnd(cleared, bitEarned) |
...
| **WaveSpawner** | 보스 Incursion 감지(IsBossWave), Incursion 클리어 시 타워 드롭 | StageData의 스케일링 배율을 Node.Initialize에 전달 |
| **Tower** | Level 1~4 지원, TryMerge/LevelUp 메서드 보유 | RunModifiers 적용 |
| **TowerInventory** | 최대 8슬롯, InventorySlot(data, level) | Incursion 클리어 시 랜덤 타워 1기 드롭 |
| **HubUI** | Floor 드롭다운(Dropdown), 스킬 트리, 방치 Soul 알림 | RefreshStageDropdown()으로 해금된 Floor 표시 |

### 0.2 핵심 발견: 코드 변경 최소화 가능 포인트

1. **멀티 Floor**: `GameManager.stages[]`에 StageData SO를 추가하면 즉시 동작. HubUI 드롭다운도 자동 반영.
2. **타워 레벨업**: `Tower.LevelUp()`, `Tower.TryMerge()` 이미 구현. 인게임 Soul 소모 업그레이드는 UI+호출 로직만 추가.
...
GDD v3.0에서 10개 Floor를 설계했으나, Phase 3에서는 **Floor 1~3을 완성**하고 이후 Floor는 Phase 4에서 확장한다. "초반 30분 재미 극대화"에 집중.

### 1.2 Floor 구성

| Floor | ID | 이름 | Incursion 수 | HP 배율 | 속도 배율 | Soul 드롭 배율 | 기지 HP | 클리어 보상 (Core Fragment) | 테마 |
|---------|-----|------|---------|---------|---------|-------------|---------|-----------------|------|
...
### 1.3 Floor 1 Incursion 구성 (기존 유지, GDD 기준 보정)

현재 Wave_01~05 모두 Bit Node만 사용. GDD 기준으로 Quick/Heavy Node 혼합 필요.

| Incursion | Node 구성 | 수량 | 스폰 간격(초) | 대기 시간(초) | 설계 의도 |
|--------|----------|------|-------------|-------------|----------|
...
| W5 (보스) | Bit x8 + Quick x4 + Heavy x2 + **Boss(Bit Lord) x1** | 15 | 0.6 | 3 | 보스 Incursion |

**변경 필요 사항**: Wave_03~05의 spawnGroups에 Quick/Heavy Node 추가. Wave_05에 Boss Node 추가.

### 1.4 Floor 2 Incursion 구성 (신규)

| Incursion | Node 구성 | 수량 | 스폰 간격(초) | 대기 시간(초) | 설계 의도 |
|--------|----------|------|-------------|-------------|----------|
| W1 | Bit x8 | 8 | 1.0 | 2 | Floor 1보다 시작부터 밀도 높음 |
...
| W5 | Bit x10 + Quick x5 + Heavy x3 | 18 | 0.6 | 3 | 혼합 Incursion. 전략 테스트 |
...
|---------|----------|----------|
| 1 | 시작 시 해금 | - |
| 2 | 누적 Soul >= 500 AND 총 킬 >= 100 | Floor 1을 3~5회 반복하면 자연 달성 |
| 3 | 누적 Soul >= 3,000 AND 총 킬 >= 500 | Floor 1~2를 10~15회 반복하면 자연 달성 |
...
| 에셋 | 경로 | 내용 |
|------|------|------|
| Stage_02.asset | ScriptableObjects/Stages/ | Memory Block, Incursion 7개, HP배율 1.8 |
| Stage_03.asset | ScriptableObjects/Stages/ | Cache Layer, Incursion 8개, HP배율 3.0 |
| Wave_S1_01~05.asset | ScriptableObjects/Waves/ | Floor 1 Incursion (기존 Wave_01~05 수정 또는 재생성) |
| Wave_S2_01~07.asset | ScriptableObjects/Waves/ | Floor 2 Incursion 7개 |
| Wave_S3_01~08.asset | ScriptableObjects/Waves/ | Floor 3 Incursion 8개 |
...
- Floor 1 Boss: 300 * 1.0 = HP 300
- Floor 2 Boss: 300 * 1.8 = HP 540
- Floor 3 Boss: 300 * 3.0 = HP 900
...
| Floor 1 Wave_03~05 수정 | SO 수정 | SO만 | spawnGroups에 Quick/Heavy/Boss 추가 |
| Floor 해금 조건 변경 | MetaManager.cs | 코드 | 누적 Soul/킬 수 기반 해금 로직 추가 |
...
GDD v3.0에서 이미 상세 설계 완료. Phase 3에서는 **Floor 1~3의 보스 3종**을 구현.
...
2. 대형 파괴 이펙트
3. Core Fragment 보상 팝업 (+N Core Fragment)
4. Floor 클리어 판정
...
```
[0:00] 타이틀 화면 -> "시작" 클릭
[0:03] Sanctum 화면 스킵, 바로 Floor 1 진입
       - 첫 플레이 감지: currentStageIndex == 0 && totalBit == 0
       - "타워가 적을 잡습니다!" 1줄 텍스트 오버레이 (2초 표시 후 자동 소멸)
[0:05] Incursion 1 시작. Arrow 3기 자동 배치(기존 FTUE 로직).
       Node 5마리가 천천히 이동. 타워가 자동 공격.
[0:30] Incursion 2. 더 많은 Node. 기지 HP가 줄기 시작.
[0:45] Incursion 3. Quick Node 등장. "빠른 적이다!" 느낌.
[1:00] 기지 HP 0. 첫 사망.
       - 런 종료 패널: "패배" + 획득 Bit 표시
       - "Hub로 이동하여 강화하세요!" 가이드 텍스트 (첫 사망 시만)
[1:10] Sanctum 화면. 스킬 트리에서 첫 업그레이드 구매.
       - 구매 가능한 스킬에 펄스 강조 이펙트 (첫 방문 시만)
[1:15] "출격" 클릭. Floor 1 재도전.
       - "강해진 것 같다!" 체감. 더 오래 생존.
[1:30] 두 번째 사망 (Incursion 3~4에서).
       - 더 많은 Bit로 더 많은 업그레이드.
[2:00] 3~5회차. 성장 루프 체득 완료.
[2:30] Incursion 5(보스) 도달. 보스와의 첫 대면.
[3:00] 보스 처치 성공. Floor 1 클리어! Core Fragment 획득!
       - "새 Floor 해금!" 알림
```
...
| 트리거 | 가이드 텍스트 | 표시 조건 |
|--------|-------------|----------|
| 첫 런 시작 | "타워가 적을 자동으로 공격합니다" | totalBit == 0, 첫 Incursion 시작 시 |
| Incursion 클리어 보상 | "타워를 획득했습니다! 드래그하여 배치하세요" | 첫 타워 드롭 시 |
| 첫 사망 | "성장하면 더 강해집니다 -- Hub에서 업그레이드!" | 첫 RunEnd 시 |
| Sanctum 첫 진입 | "스킬을 눌러 영구 강화를 구매하세요" | Sanctum 첫 표시 시 |
| 보스 등장 | "보스가 나타났습니다! 처치하면 클리어!" | 보스 Incursion 시작 시 |

**구현 방식**: `FtueManager` (MetaManager에 ftue 플래그 저장) -> InGameUI/HubUI에서 가이드 텍스트 표시

### 4.4 첫 런 클리어 보상

Floor 1 클리어 시 특별 보상으로 성장 루프 체감 극대화:

| 보상 | 양 | 효과 |
|------|-----|------|
| Core Fragment | 2 (기본) | 스킬 트리에서 첫 Core Fragment 노드 구매 가능 |
| Bonus Bit | 100 | "클리어 보너스!" 팝업으로 추가 지급 |
| 신규 Floor 해금 알림 | - | "Floor 2: Memory Block 해금!" (조건 충족 시) |
...
| 첫 플레이 시 Sanctum 스킵 로직 | GameManager.cs | 코드 (소) | Start()에서 분기 |
...
Floor 3에서 등장하는 Shield Node 상세 설계.
...
| P1 | MetaManager Floor 해금 조건 변경 | 중 | 누적 Soul/킬 기반 |
...
| 우선순위 | 작업 | 공수 | 상세 |
|---------|------|------|------|
| P0 | Floor 1 Wave_03~05 SO 수정 | 소 | Quick/Heavy/Boss Node 추가 |
| P0 | Boss Node SO 3종 생성 | 소 | Node_Boss_BitLord/RushKing/IronWall |
| P0 | Shield Node SO 생성 | 소 | Node_Shield |
| P0 | Stage_02, Stage_03 SO 생성 | 소 | Incursion 배열 연결 |
| P0 | Floor 2 Incursion SO 7개 생성 | 중 | Wave_S2_01~07 |
| P0 | Floor 3 Incursion SO 8개 생성 | 중 | Wave_S3_01~08 |
...
| P1 | Floor 해금 알림 UI | 소 | 런엔드 패널에 "새 Floor 해금!" 표시 |
| P2 | 보스 처치 연출 UI (Core Fragment 보상 팝업) | 소 | "+2 Core Fragment" 팝업 |
...
| P2 | Floor 2/3 시각 테마 변경 | 중 | 맵 컬러 팔레트, 배경 변화 |

### 6.5 QA팀장 (unity-qa-engineer)

| 우선순위 | 작업 | 공수 | 상세 |
|---------|------|------|------|
| P0 | Floor 1 보스 Incursion 밸런스 테스트 | 중 | 3~5회차에 클리어 가능한지 |
| P0 | Floor 2/3 Incursion 난이도 검증 | 중 | 적절한 사망/클리어 비율 |
| P1 | 타워 업그레이드 기능 테스트 | 소 | Bit 차감, 레벨업 동작 |
| P1 | Floor 해금 조건 테스트 | 소 | 누적 조건 충족 시 정상 해금 |
...
| P2 | Floor 2/3 전투 BGM 변주 | 중 | Floor별 분위기 차별화 |
...
| 4 | Floor 1 Incursion 수정 + Floor 2/3 SO 생성 | 기획 | 3 완료 | 중 |
| 5 | 보스 HP 바 UI + 타워 업그레이드 UI | UI | 1, 2 완료 | 중 |
| 6 | Floor 해금 조건 변경 | 프로그래밍 | 4 완료 | 중 |
...
- **SO 주도 설계**: 대부분의 콘텐츠(Floor, Incursion, Node, 타워 수치)는 SO 에셋만으로 확장 가능
...
- Floor 1 보스(HP 300)가 초보자에게 너무 어렵거나 쉬울 수 있음 -> QA 검증 후 SO 수치 조정
- Floor 2/3 스케일링(x1.8, x3.0)이 영구 업그레이드 진행 속도와 맞지 않을 수 있음 -> 테스트 플레이 필수
