...
- 공통 HUD:
- 상단 바 배경, HP 바 프레임/필, Soul/Core Fragment 아이콘, Incursion 배지
- 인게임:
- 타워 인벤토리 슬롯, 드래그 하이라이트(배치/합성/판매), 배속 버튼(x1/x2/x3), 일시정지 버튼
- Sanctum:
- 스킬 노드(잠김/해금가능/해금완료), 연결선(비활성/활성), 노드 구매 팝업 프레임
- 결과 화면:
- `RUN COMPLETE`, `STAGE CLEAR` 배너, Core Fragment 보상 카드, 신규 해금 배지
- 시스템:
...
- 마이크로: 적 처치 파편, Soul 숫자 팝업
- 소형: Incursion 클리어 플래시
- 중형: 영구 업그레이드 구매 이펙트
- 대형: Floor 클리어, Core Fragment 획득 연출
...
| Progress | Incursion 시작/클리어 | 각 2~3 |
| Progress | Floor 클리어/Core Fragment 획득 | 각 2~3 |
...
2. Floor 1/2 배경 + Incursion/런 종료 UI
3. Cannon/Ice + Shield/Swarm + 클리어 연출
4. Sanctum 스킬 트리 노드 상태 3종 + 구매 연출
